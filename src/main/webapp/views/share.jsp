...
<div class="card-cute">
    ...
    <div class="text-center mb-4">
        ...
        <h3 class="fw-bold mb-1">Chia sẻ video</h3>
        <p class="text-muted small mb-0">Gửi video này cho bạn bè của bạn 🌷</p>
    </div>
    ...
    <% if (video != null) { %>
        <div class="video-box mb-4">
            <div class="fw-bold text-dark mb-1">
                <i class="fa-solid fa-clapperboard me-2 text-danger"></i>
                <%= video.getTitle() %>
            </div>
    ...
    <form action="<%= request.getContextPath() %>/share" method="post">
        ...
        <% } else { %>
            <input type="hidden" name="videoId" value="<%= request.getParameter("id") %>">
        <% } %>
        ...
        <div class="mb-4">
            ...
            <input type="email"
                   name="email"
                   class="form-control"
                   placeholder="Nhập email muốn chia sẻ..."
                   required>
        </div>
    ...
    <div class="text-center mt-4">
        <% if (video != null) { %>
            <a href="<%= request.getContextPath() %>/video-detail?id=<%= video.getId() %>" class="cute-link">
                ← Quay lại video
            </a>
        <% } else { %>
            <a href="<%= request.getContextPath() %>/home" class="cute-link">
                ← Về trang chủ
            </a>
        <% } %>
    </div>
